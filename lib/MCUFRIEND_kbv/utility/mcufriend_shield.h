...
{
#ifdef FAST_PORT_MASK
	GPIO.out_w1tc = port_mask[0xFF];  //could define once as DMASK
	GPIO.out_w1ts = port_mask[data];
#else
...
static void setWriteDir()
{
	...
	gpio.pin_bit_mask = port_mask[0xFF];
...
#ifdef FAST_PORT_MASK
#define write8(x)     { GPIO.out_w1tc = clear_mask; GPIO.out_w1ts = port_mask[x]; GPIO.out_w1ts = (1 << WR_PIN); }
...
#define CS_ACTIVE		GPIO.out1_w1tc.data = (1 << (CS_PIN-32))
#define CS_IDLE			GPIO.out1_w1ts.data = (1 << (CS_PIN-32))
#define RESET_ACTIVE	GPIO.out1_w1tc.data = (1 << (RESET_PIN-32))
#define RESET_IDLE		GPIO.out1_w1ts.data = (1 << (RESET_PIN-32))
#else
#define write8(x)     { write_8(x); WR_STROBE; }
#endif // FAST_PORT_MASK
