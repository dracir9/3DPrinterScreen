...
//
// Uncomment to support the following tft drivers
...
// Other Settings
//#define ESP32
#define FAST_PORT_MASK			//Trade memory for speed - Uses 1kbyte on 32bit CPU
...
#ifdef FAST_PORT_MASK
	for (uint16_t c = 0; c < 256; c++)
	{
		port_mask[c] = 0;
		if (c & 0x01) port_mask[c] |= (1 << LCD_D0);
		if (c & 0x02) port_mask[c] |= (1 << LCD_D1);
		if (c & 0x04) port_mask[c] |= (1 << LCD_D2);
		if (c & 0x08) port_mask[c] |= (1 << LCD_D3);
		if (c & 0x10) port_mask[c] |= (1 << LCD_D4);
		if (c & 0x20) port_mask[c] |= (1 << LCD_D5);
		if (c & 0x40) port_mask[c] |= (1 << LCD_D6);
		if (c & 0x80) port_mask[c] |= (1 << LCD_D7);
	}
	clear_mask = port_mask[0xFF] | (1 << WR_PIN);
#endif // FAST_PORT_MASK
}

static uint8_t done_reset, is8347;
#ifdef SUPPORT_9488_555
static uint8_t is555;
...
//	    if (_lcd_ID	== 0x1963) WriteCmdParamN(0x13, 0, NULL);   //NORMAL mode
    ...
    // cope with 9320 variants
...
#if defined(SUPPORT_0139) || defined(SUPPORT_0154) || defined(SUPPORT_9225)
          common_ORG:
#endif
            ORG = (val & 0x20) ? (1 << 3) : 0;
#ifdef SUPPORT_8230
        ...
        case 0x1289:
            ...
            _lcd_drivOut = GS | SS_v | (_lcd_rev << 13) | 0x013F;      //REV=0, BGR=0, MUX=319
            if (val & 0x08)
...
#endif
    if (_lcd_capable & MIPI_DCS_REV1) {
		if (!(prev_SC == x && prev_EC == x1))
		{
			prev_SC = x;
			prev_EC = x1;
			WriteCmdParam4(_SC, x >> 8, x, x1 >> 8, x1);   //Start column instead of _MC
		}
		if (!(prev_SP == y && prev_EP == y1))
		{
			prev_SP = y;
			prev_EP = y1;
...
#if defined(SUPPORT_0154) || defined(SUPPORT_9225)
